...
func (mq *MetaQuery) QueryBlock() *BlockQuery {
...
func (mq *MetaQuery) WithBlock(opts ...func(*BlockQuery)) *MetaQuery {
	query := (&BlockClient{config: mq.config}).Query()
...
func (mq *MetaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Meta, error) {
	...
	if query := mq.withBlock; query != nil {
		if err := mq.loadBlock(ctx, query, nodes, nil,
			func(n *Meta, e *Block) { n.Edges.Block = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MetaQuery) loadBlock(ctx context.Context, query *BlockQuery, nodes []*Meta, init func(*Meta), assign func(*Meta, *Block)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Meta)
	for i := range nodes {
		if nodes[i].meta_block == nil {
			continue
		}
		fk := *nodes[i].meta_block
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "meta_block" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
