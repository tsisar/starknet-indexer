...
func (tx *Tx) init() {
	...
	tx.PositionActivity = NewPositionActivityClient(tx.config)
	tx.ProtocolStat = NewProtocolStatClient(tx.config)
	tx.StableSwapStat = NewStableSwapStatClient(tx.config)
	tx.SwapEvent = NewSwapEventClient(tx.config)
